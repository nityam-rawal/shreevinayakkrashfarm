...
export const RISK_RULES: RiskRule[] = [
  ...
  { fc: "reserved_stock", pri: 10, severity: "note", label: "Reserved stock", ask: "Kuch stock reserved hai — available quantity kam hai.",
    test: /\breserved\b/i },
  { fc: "location_stock", pri: 5, severity: "ask", label: "Godown transfer", ask: "Warehouse se shop bheja — ye purchase nahi, transfer hai. Transfer maane?",
    test: /\b(warehouse|godown)\b[^.]*\bto\b[^.]*\bshop\b|\bnot\s*a\s*purchase\b/i },
  ...
  { fc: "subcontract", pri: 5, severity: "ask", label: "Job work", ask: "Job worker ko maal bheja — issue aur return alag likhein?",
  ...
  { fc: "expiry", pri: 6, severity: "ask", label: "Expiry", ask: "Expiry wale batch alag handle karein — sale na karein?",
    test: /\bexpir\w*\b/i },
  { fc: "batch", pri: 5, severity: "note", label: "Batch", ask: "Same item ke 2 batch hain — kaun sa batch?",
    test: /\b(two|2|multiple|kai)\s*batch(?:es)?\b|\bbatch(?:es)?\b[^.]*\bdifferent\b|\bbatch\s*no/i },
  ...
  { fc: "fx", pri: 5, severity: "note", label: "FX settlement", ask: "Foreign invoice aur INR settlement dono track honge.",
    test: /\b(exchange\s*rate|forex|fx)\b|\b(usd|eur|gbp)\b[^.]*\bbank\s*debit\b|\binr\s*equivalent\b/i },
  ...
  { fc: "wrong_item", pri: 5, severity: "ask", label: "Galat item", ask: "Invoice ka item aur aaya item alag hai — kaun sa likhein?",
    test: /\binvoice\s*says\b(?![^.]*\b(?:\d+\s*)?units?\b)[^.]*\bbut\b[^.]*\breceived\b|\bwrong\s*item\b|\bdifferent\s*item\b/i },
